...
char const*const FLASHLIGHT_FILE = "/sys/class/leds/flashlight/brightness";
        ...
        struct light_state_t const* state)
{
	...
	if (g_kbd_on < 0 ||
	    ...
	    (g_kbd_on > 0 && !kbd_on))
	{
		err = write_int(KEYBOARD_FILE, kbd_on ? g_brightness : 0);
	}
